...
async def api_order():
    try:
        ...
        # Сообщение клиенту через PAY бота
        ...
        # Сообщение админу через STOCK бота
...
async def handle_admin_decision(callback_query: types.CallbackQuery):
    try:        action, user_id, order_id = callback_query.data.split('_', 2)
...
async def main():
    if os.getenv('RENDER'):
        logger.info("Запущено на Render - только Flask")
        return
